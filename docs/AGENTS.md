...
1. `.claude/agents/<name>.md` を新規作成。既存のコアエージェント（`knowledge.md` など）をひな形にコピーするのが速い。
2. frontmatter を書く：
   - `name`: ファイル名と一致
...
エージェントは自分の領分を超えたら、**チャット内で抱え込まず**タスクを作って他エージェントに渡す（`04_Tasks/` にタスク化し担当・コメントを記入）。これで仕事がチャットに埋もれず追跡可能になる。`orchestrator` がその交通整理をする。
