...
> Nine specialized agents, a type-driven PARA knowledge base, scheduled automation, and—most importantly—**guardrails that decide what the AI must *not* do.**
...
└── .claude/
    ├── agents/      ← the 9 agent definitions (the source of truth)
...
| 1 | `reviewer` | Double-checks other agents' output (read-only) |
| 1 | `relationship` | Organizes relationship facts & reflection — **never ghostwrites** (read-only) |
| 1 | `self-analysis` | Surfaces patterns from reflection logs, asks questions |
| 1 | `finance` | Household visibility, fixed-cost review (never executes payments) |
| 1 | `planner` | Research/compare/schedule for trips & big purchases (never books) |
| 2 | `english` | Writing correction & vocabulary notes (IELTS-oriented) |
| 2 | `knowledge` | Turns URLs/content into linked knowledge notes; weekly graph upkeep |
| 3 | `infrastructure` | Audits the task DB, finds duplicates & silent failures (proposals only) |

**Agents = unattended & routine. Skills = interactive & needs confirmation.** Sensitive work (weekly review, task audit, message review, reflection) runs as a *skill* in the foreground so a human stays in the loop.
...
5. **Tune the agents.** The 9 files in `.claude/agents/` are the source of truth for behavior. Edit triggers/constraints to taste.
...
> 9体の専門エージェント、type駆動のPARAナレッジベース、スケジュール自動化、そして何より **「AIにやらせないこと」を決めるガードレール** が核です。
...
- **信頼は制約から生まれる** — 一番効いている設計は「AIに**禁止**していること」。パートナー宛の文章は代筆しない、お金は動かさない、健康判断はしない――全部人間の領分。複数のエージェントはツール権限で読み取り専用にしてあり、境界が**技術的に強制**されている。
...
| 1 | `reviewer` | 他エージェント出力のダブルチェック（読み取り専用） |
| 1 | `relationship` | 関係の事実整理・内省の壁打ち。**代筆は一切しない**（読み取り専用） |
| 1 | `self-analysis` | 内省ログからパターン抽出・問いの提示 |
| 1 | `finance` | 家計の見える化・固定費棚卸し（支払いは代行しない） |
| 1 | `planner` | 旅行・大型購入の調査/比較/計画（予約は代行しない） |
| 2 | `english` | ライティング添削・語彙ノート（IELTS志向） |
| 2 | `knowledge` | URL/コンテンツをリンク付きナレッジ化・週次グラフ整備 |
| 3 | `infrastructure` | タスクDB点検・重複/静かな故障の検出（提案のみ） |

**エージェント＝無人・定型／スキル＝対話・要確認。** センシティブな作業（週次レビュー・タスク棚卸・メッセージ振り返り・内省）は人間が必ず関与できるよう、前面の **スキル** として動かします。
...
5. **エージェントを調整** — `.claude/agents/` の9ファイルが挙動の権威ソース。トリガー・制約を好みに編集。
