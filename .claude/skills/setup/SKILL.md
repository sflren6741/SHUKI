...
**プレースホルダーが多数残っている（3箇所以上）** → **初回セットアップ**
...
**プレースホルダーがほぼない（2箇所以下）** → **再実行（アップデートモード）**
全フェーズを流さず、以下のメニューを出す：
```
✅ すでに設定済みのようです。何を更新しますか？（複数選択可）

1. プロフィール（Identity.md）
2. 価値観・原則（哲学・価値観.md）
3. Areas（追加・変更・削除）
4. パス設定（CLAUDE.md）
5. Obsidianプラグイン設定
...
7. 専門エージェント
```
番号を答えてもらい、選んだフェーズ（Phase 2〜8 の対応箇所）だけを実行する。「全部」と言われた場合は Phase 2〜8 を順番に流す。
