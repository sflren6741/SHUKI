...
**JP** — 何でもここに放り込む（メモ・URL・思いつき）。`orchestrator` が定期的に振り分け、frontmatter を補完して正式な置き場所（タスク／ログ／ナレッジ／プロジェクト）へ移す。`status: done` にするのは情報を正式ファイルに**移行し終えてから**。判断軸は「消すか」ではなく「内容が転記済みか」。

> URL capture: set `type: 🔗URL` + `url:` and the knowledge agent turns it into a linked note. Skeleton: `インボックス/_example-inbox.md`.
