...
**JP** — 継続的な責任領域。各 Area は **単一ハブノート** `<area>.md` のみで表す（サブフォルダを作らない・例外なし）。ハブは散文＋Bases で `area` 一致ノートを集約。派生コンテンツは Tasks / Logs / Resources に振り分ける。各ノートの `area:`（プレーンテキスト）でこのハブに自動で紐づく。

> Define your own areas (work, health, money, relationships, …). Skeleton: `_example-area.md`.
