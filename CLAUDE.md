...
10. **イントラファイルの継続更新** — 会話中にオーナーの自己言及情報が自然に出てきた場合（職業・役割の変化・新しいArea追加・価値観の言語化・禁止ゾーンの変化・キーパーソンの追加など）、会話末に更新提案を1行出す（例：「Identity.md の〈職業〉欄を更新しますか？」）。確認が取れたら Edit で書き込む。確認なく書き換えない。
